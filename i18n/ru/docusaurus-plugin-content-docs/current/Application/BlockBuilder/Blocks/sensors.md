...
- Используйте репортер-блоки датчиков с `if`, `if/else` и `wait until` для реактивной логики.
- Комбинируйте значения датчиков с блоками Operators для построения порогов и условий.
- Проверяйте `Device type`, прежде чем отправлять команды, зависящие от конкретного подключенного оборудования.
