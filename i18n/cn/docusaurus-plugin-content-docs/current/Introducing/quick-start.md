...
- 使用账户后，你的配置文件和设置会在所有设备间同步。
- 不使用账户时，数据只保存在当前设备。
- 如果在无账户模式下重装应用，你的配置文件和设置会丢失。

你仍然可以使用 **Continue without an account**，但不建议长期使用。
...
:::warning Important
预置配置可用于实验修改，但你的更改不会保存。下次打开时会恢复为原始状态。
:::
...
:::info Important
默认每个配置最多可添加 **2** 个 Hub。若要扩展到 **5** 个，需要购买额外 Hub 插槽。
:::
